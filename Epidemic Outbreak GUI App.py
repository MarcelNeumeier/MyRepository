...
class Simulation():
    """A class to control the simulation and help faciliate in the spread of the disease."""
    ...
    def __init__(self):
        ...
        #Population size must be a perfect square for this programm
...
class Person():
    ...
    def heal(self):
        """Heal a person from an infection"""
    ...
    def update(self, simulation):
        ...
        If they are, increase the days infected cout, then chek if they should die or be healed."""
                ...
                if self.population[i][j].is_dead == False:
                    ...
                    #If i == simulation.grid_size -1 we are in the last row, so we can't look below
                        ...
                        #If we are in any other column, we can look left, right or above
                            ...
                            if self.population[i][j-1].is_infected or self.population[i-1][j].is_infected:
                                self.population[i][j].infect(simulation)
                    #Otherwise, we are in a row inbetween, we can look left, right, below or above
                    else:
                        #If j == 0, we are in the first column, so we can't look left.
                        if j == 0:
...
#Set the inital conditions of the population
...
for i in range(1, sim.sim_days):
    ...
    #Update the tkinter window to relfect the graphics change
    sim_window.update()

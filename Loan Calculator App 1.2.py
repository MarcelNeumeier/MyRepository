#Function Challenge 35: Loan Calculator App
...
def show_loan_info(loan, number):
    ...
    print("\n----Loan inforamtion after " + str(number) + " months----")
    for key, value in loan.items():
        print(key.title() + ": " + str(value))

def collect_interest(loan):
    """Update loan for interst per month"""
    #Divide by 12 to simulate interest monthly
    loan["principal"] = loan["principal"] + loan["principal"]*loan["rate"]/12    

def make_monthly_payment(loan):
    """Simulate making a monthly payment to down the pincipal"""
    ...
    #You are not required to make full payment this month, you have payed off your loan
    else:
        #For this else block, loan["pricipal"] will be negative
...
def create_graph(data, loan):
    ...
    #Create a plot for x values and y_values (month number and principal)
...
while my_loan["principal"] > 0:
    ...
    #Increment month number, collect interest, make a payment, add data to plot and show loan
    month_number +=1
...
#The loan can NEVER be paid off, can't get ahead of interest
    ...
    print("You cannot get ahead of the interst! :-(")
